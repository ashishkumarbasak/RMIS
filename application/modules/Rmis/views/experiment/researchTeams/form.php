...
<form name="research_info" id="research_info" method="post" action="">
	...
	<div class="main_form">
		<div class="left_form">
        	<div class="form_element">
                ...
                <div class="field">
              		<input type="text" name="team_formation_date" id="team_formation_date" data-date-format="yyyy-mm-dd"  value="<?php if($researchTeam){ if($researchTeam->team_formation_date=='0000-00-00'){echo '';} else {echo $researchTeam->team_formation_date;}}?>" class="textbox disabled" readonly="readonly">
	...
	<div class="form_element">
    	<div class="button_panel" style="margin-right:15px;">
    		<?php if(isset($experiment_detail) && ($researchTeam!=NULL || $teamMembers!=NULL)) { ?>
...
$(document).ready(function () {
	var crudServiceBaseUrl = "<?php echo base_url();?>",
		dataSource = new kendo.data.DataSource({
			transport: {
				read:  {
					url: crudServiceBaseUrl + "rmis/experiment/researchTeams/getResearchTeamInformation/<?php if(isset($experiment_detail) && $experiment_detail->experiment_id!=NULL) echo $experiment_detail->experiment_id; else echo 0; ?>"
				},
				update: {
					url: crudServiceBaseUrl + "rmis/experiment/researchTeams/updateMembers",
					dataType: 'json',
					type: 'POST',
				},
				destroy: {
					url: crudServiceBaseUrl + "rmis/experiment/researchTeams/destroyMembers",
					dataType: 'json',
					type: 'POST',
				},
				create: {
					url: crudServiceBaseUrl + "rmis/experiment/researchTeams/addMembers",
					dataType: 'json',
					type: 'POST'
				},
			...
			schema: {
				model: {
					...
					fields: {
						...
						ExperimentID: { editable: false, nullable: true },
...
function memberTypeDropDownEditor(container, options) {
	...
	$('<input required data-text-field="MemberType" data-value-field="MemberTypeID" data-bind="value:MemberTypeID"/>')
		...
		.kendoDropDownList({
			...
			dataSource: {
				transport: {
					read: ServiceBaseUrl + "rmis/experiment/researchTeams/getListofMemberTypes"
				}
			},
			select: function(e){
				var dataItem = this.dataItem(e.item.index());
