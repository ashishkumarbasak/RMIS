...
class ImplementationCommittees extends MX_Controller{
    ...
    public function index($program_id=NULL){
        ...
        if($program_id!=NULL){
			...
			if($this->input->post('delete_program_implcommittee')){
				$request = json_encode($this->input->post());
				$this->dataDestroy($request);
				redirect('Rmis/Program/ImplementationCommittees/'.$program_id,'refresh');
			}
	...
	public function dataDestroy($request=NULL){
		if($request!=NULL){
			 $request = json_decode($request);
			 $data = $this->grid->destroy('rmis_program_implementation_committees', $request, 'program_id');
			 $data = $this->grid->destroy('rmis_program_implementation_committee_members', $request, 'program_id');
		}
		else{   
			header('Content-Type: application/json');
			$request = json_decode(file_get_contents('php://input'));
			$data = $this->grid->destroy('rmis_program_implementation_committees', $request->models, 'program_id');
			$data = $this->grid->destroy('rmis_program_implementation_committee_members', $request->models, 'program_id');
			echo json_encode($data , JSON_NUMERIC_CHECK);
		}        
	}	
