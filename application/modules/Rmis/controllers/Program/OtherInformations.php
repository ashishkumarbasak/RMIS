...
class OtherInformations extends MX_Controller{
    ...
    public function index($program_id=NULL){
        ...
        if($program_id!=NULL){
			...
			if($this->input->post('delete_program_otherinfo')){
				$request = json_encode($this->input->post());
				$this->dataDestroy($request);
				redirect('Rmis/Program/OtherInformations/'.$program_id,'refresh');
			}
    ...
    public function dataUpdate($request){
	 ...
	 public function dataDestroy($request=NULL){
		if($request!=NULL){
			 $request = json_decode($request);
			 $data = $this->grid->destroy('rmis_program_other_informations', $request, 'id');
		}
		else{   
			header('Content-Type: application/json');
			$request = json_decode(file_get_contents('php://input'));
			$data = $this->grid->destroy('rmis_program_other_informations', $request->models, 'id'); 
			echo json_encode($data , JSON_NUMERIC_CHECK);
		}        
	}	
