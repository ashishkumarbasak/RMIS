...
class Informations extends MX_Controller{
    ...
    public function index($program_id=NULL){
		...
		if($program_id!=NULL){
			...
			if($this->input->post('delete_program_information')){
				...
				redirect('Rmis/Program/Informations','refresh');
			}
	...
	public function dataDestroy($request=NULL){
		if($request!=NULL){
			 $request = json_decode($request);
			 $data = $this->grid->destroy('rmis_program_informations', $request, 'program_id');
		}
		else{   
			header('Content-Type: application/json');
			$request = json_decode(file_get_contents('php://input'));
			$data = $this->grid->destroy('rmis_program_informations', $request->models, 'program_id'); 
			echo json_encode($data , JSON_NUMERIC_CHECK);
		}        
	}	              
}
