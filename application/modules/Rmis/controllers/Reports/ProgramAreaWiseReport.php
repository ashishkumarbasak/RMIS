<?php
class ProgramAreaWiseReport extends MX_Controller
{
    private $_data;
    ...
    public function index()
    {
        ...
        $this->template->build('reports/program_area_wise_report');
    }
    ...
    public function getReport($mode = 'preview')
    {
		...
		$program_department_name 			= $this->input->post('program_department_name', true);
        ...
        if (!empty($program_department_name) ) {
            $whereCond .= " AND hrm_organograms.id = ".$program_department_name;
        }
