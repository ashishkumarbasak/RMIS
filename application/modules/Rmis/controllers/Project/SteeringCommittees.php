...
class SteeringCommittees extends MX_Controller{
    ...
    public function index($program_id=NULL, $project_id=NULL){
        $this->template->title('Research Management(RM)', ' Programs', ' Project Steering Committee Information');
		...
		if($project_id!=NULL){
			...
			if($this->input->post('delete_steering_committee')){
				...
				redirect('Rmis/Project/SteeringCommittees/'.$program_id."/".$project_id,'refresh');
			}
			...
			$steeringCommittee = $this->project->get_steeringCommittee($project_id);
			$this->template->set('steeringCommittee', serialize($steeringCommittee));
			
			$teamMembers = $this->project->get_steeringCommitteeTeamMembers($project_id);
        ...
        $this->template->set('content_header_title', 'Project Steering Committees Information');
        ...
        $breadcrumb = '<ul class="breadcrumb">
						...
						<li><a href="#">Project info.</a><span class="divider">&raquo;</span></li><li class="active">Project Steering Committee Information</li>
					  </ul>';
        $this->template->set('breadcrumb', $breadcrumb);		
        $this->template->set_partial('steeringCommitteeForm','project/steeringCommittees/form');
		$this->template->set_partial('tab_menu','project/form_tabs');
        $this->template->set_partial('sidebar', 'layouts/sidebar',$_data)
               ->build('project/steeringCommittees/index');
    }
	...
	public function dataCreate($request){
		...
		$data = $this->grid->create('rmis_project_steering_committees', $columns, $request, 'id'); 
		...
		if(!empty($request->member_names)>0){
			foreach($request->member_names as $team_member_key=>$team_member_name){
				if($team_member_name!=NULL){
					...
					$this->grid->create('rmis_project_steering_committee_members', $columns, $request, 'id');
					$i++;	
				}
			}
		}
	...
	public function dataUpdate($request){
		...
		$data = $this->grid->update('rmis_project_steering_committees', $columns, $request, 'project_id'); 
        
		$columns = array('committee_member_type','member_name','designation', 'contact_no', 'email', 'project_id');
		$i=0;
		if(!empty($request->member_names)>0){
			$this->project->clean_projectSteeringCommitteeMembers($request->project_id);
			foreach($request->member_names as $team_member_key=>$team_member_name){
				if($team_member_name!=NULL){
					$request->member_name = $team_member_name;
					$request->designation = $request->designations[$i];
	...
	public function dataDestroy($request=NULL){
		if($request!=NULL){
			 $request = json_decode($request);
			 $data = $this->grid->destroy('rmis_project_steering_committees', $request, 'project_id');
			 $data = $this->grid->destroy('rmis_project_steering_committee_members', $request, 'project_id');
		}
		else{   
			header('Content-Type: application/json');
			$request = json_decode(file_get_contents('php://input'));
			$data = $this->grid->destroy('rmis_project_steering_committees', $request->models, 'project_id');
			$data = $this->grid->destroy('rmis_project_steering_committee_members', $request->models, 'project_id');
			echo json_encode($data , JSON_NUMERIC_CHECK);
		}        
	}	
	...
	public function deleteTeamMember(){
		...
		$this->project->deleteSteeringCommitteeTeamMemberFromProgram($team_member_id,$project_id);
	}
