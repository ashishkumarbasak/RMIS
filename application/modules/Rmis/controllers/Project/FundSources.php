...
class FundSources extends MX_Controller{
		...
		$this->template->append_metadata('<script src="/assets/js/custom/rmis_setup.js"></script>');
       	...
       	if($project_id!=NULL){
			...
			if($this->input->post('delete_fundSources')){
				...
				redirect('Rmis/Project/FundSources'.$program_id."/".$project_id,'refresh');
			}
	...
	public function dataDestroy($request=NULL){
		if($request!=NULL){
			 $request = json_decode($request);
			 $data = $this->grid->destroy('rmis_project_cost_estimations', $request, 'project_id');
			 $data = $this->grid->destroy('rmis_project_cost_breakdowns', $request, 'project_id');
			 $data = $this->grid->destroy('rmis_project_funding_sources', $request, 'project_id');
		}
		else{   
			header('Content-Type: application/json');
			$request = json_decode(file_get_contents('php://input'));
			$data = $this->grid->destroy('rmis_project_cost_estimations', $request->models, 'project_id');
			$data = $this->grid->destroy('rmis_project_cost_breakdowns', $request->models, 'project_id');
			$data = $this->grid->destroy('rmis_project_funding_sources', $request->models, 'project_id'); 
